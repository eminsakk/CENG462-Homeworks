...
# def Qlearning(board, alpha, gamma, epsilon, QTable, player):
#     # Find the best action according to the current Q-table.
    
    
#     if random.random() <= epsilon:
#         # Epsilon-Greedy Strategy.
        
#         # First we need to which positions are available to insert.
#         actionsIdx =  [i  for i in range(9) if board[i] == "-"]
#         actionChoice = random.choice(actionsIdx)
        
#         # Then update the board.
#         nextBoard = board[:actionChoice] + player + board[actionChoice + 1:]
#         nextAction  = actionChoice // 3, actionChoice % 3
...
#                 boardIndex = i * 3 + j
#                 if board[boardIndex] == "-":
#                     if QTable[currAction][board] > maxValue:
#                         maxValue = QTable[currAction][board]
#                         bestAction = currAction
                        
#         boardIndex = bestAction[0] * 3 + bestAction[1]
#         nextBoard = board[:boardIndex] + player + board[boardIndex + 1:]
#         nextAction = bestAction
    
    
    
#     reward = get_reward(nextBoard,player)
#     a = QTable[nextAction].values()
    
#     QTable[nextAction][board] = QTable[nextAction][board] + alpha * (reward + gamma * max(QTable[nextAction].values()) - QTable[nextAction][board]) 
...
def generate_boards():
  ...
  for board in boards:
    ...
    if xCount == oCount or xCount == oCount + 1 or xCount + 1 == oCount:    
...
print(SolveMDP("Q-learning","mdp1.txt",37))
